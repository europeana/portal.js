...
// oauth2 scheme (https://github.com/nuxt-community/auth-module/blob/master/lib/schemes/oauth2.js)
// changes to work with i18n
...
export default class Oauth2Scheme {
  ...
  get _redirectURI() {
    ...
    const localePath='/'+this.$auth.ctx.app.store.$i18n.locale;

    if (url) {

      return url;
    }
    //changed to include locale
    if (process.server && this.req) {
      const protocol = 'http' + (isHttps(this.req) ? 's' : '') + '://';

      return protocol + this.req.headers.host +localePath+this.$auth.options.redirect.callback;
    }

    if (process.client) {
      return window.location.origin +'/'+localePath+this.$auth.options.redirect.callback;
    }
    return window.location.origin +'/'+localePath+this.$auth.options.redirect.callback;
  }
  ...
  async _handleCallback() {
    ...
    // Changed to include locale
    if (this.$auth.options.redirect && normalizePath(this.$auth.ctx.route.path) !== normalizePath(localePath+this.$auth.options.redirect.callback)) {
      return;
    }
