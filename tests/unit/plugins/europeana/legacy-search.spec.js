  ...
  describe('legacyUrl()', () => {
    ...
    context('for the newspapers collection', () => {
      ...
      context('for a search filtering by a start date', () => {
        ...
        it('returns the classic portal URL for the collection search', () => {
          const redirectUrl = legacyUrl(searchParams);
          redirectUrl.should.eq('https://classic.europeana.eu/portal/en/collections/newspapers?q=test&f[api][]=default&range[proxy_dcterms_issued][begin]=1900-01-01&range[proxy_dcterms_issued][end]=*');
        });
      });
      ...
      context('for a search filtering by an end date', () => {
        ...
        it('returns the classic portal URL for the collection search', () => {
          const redirectUrl = legacyUrl(searchParams);
          redirectUrl.should.eq('https://classic.europeana.eu/portal/en/collections/newspapers?q=test&f[api][]=default&range[proxy_dcterms_issued][begin]=*&range[proxy_dcterms_issued][end]=2000-12-31');
        });
      });
      ...
      context('for a search filtering by start and end date', () => {
        ...
        it('returns the classic portal URL for the collection search', () => {
          const redirectUrl = legacyUrl(searchParams);
          redirectUrl.should.eq('https://classic.europeana.eu/portal/en/collections/newspapers?q=test&f[api][]=default&range[proxy_dcterms_issued][begin]=1900-01-01&range[proxy_dcterms_issued][end]=2000-12-31');
        });
      });
      ...
      context('for a search filtering by a specific date', () => {
        ...
        it('returns the classic portal URL for the collection search', () => {
          const redirectUrl = legacyUrl(searchParams);
          redirectUrl.should.eq('https://classic.europeana.eu/portal/en/collections/newspapers?q=test&f[api][]=default&range[proxy_dcterms_issued][begin]=1940-01-12&range[proxy_dcterms_issued][end]=1940-01-12');
        });
      });
    });
    ...
    context('for the fashion collection', () => {
      ...
      context('for a simple a fulltext search', () => {
        const searchParams = { query: 'test', qf: [fashionQfParam, 'CREATOR%3A"Chanel (Designer)"', 'CREATOR%3A"Valens (Designer)"', 'proxy_dcterms_medium.en%3A"Material%3A silk"', 'proxy_dc_format.en%3A"Technique%3A weaving techniques"', 'proxy_dc_type.en%3A"Object Type%3A ensemble"'] };
        it('returns the classic portal URL for the collection search', () => {
          const redirectUrl = legacyUrl(searchParams);
          redirectUrl.should.eq('https://classic.europeana.eu/portal/en/collections/newspapers?q=test&f[CREATOR][]=Chanel+(Designer)&f[CREATOR][]=Valens+(Designer)&f[proxy_dc_format.en][]=Technique%3A+weaving+techniques&f[proxy_dc_type.en][]=Object+Type%3A+ensemble&f[proxy_dcterms_medium.en][]=Material%3A+silk');
        });
      });
    });
