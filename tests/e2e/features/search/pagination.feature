Feature: Search pagination
  ...
  Scenario: Pagination of search results
    ...
    Then I see a `search results pagination`
  ...
  Scenario: Pagination links preserve query and facet selection.
    ...
    And I see a link to "/en/search?page=2&qf=TYPE%3A%22IMAGE%22&query=paris&view=grid" in the `search results pagination`
  ...
  Scenario: Pagination links preserve query and facet selection from the url.
    ...
    Then I see a link to "/en/search?query=paris&page=2&qf=TYPE%3A%22IMAGE%22" in the `search results pagination`
