Feature: Search pagination
  ...
  Scenario: Pagination links preserve query and facet selection.
    ...
    And I wait 1 second
    Then I see a link to "/search?page=2&qf=TYPE%3A%22IMAGE%22&query=paris" in the `pagination navigation`
  ...
  Scenario: Pagination links work when the page was accessed from the url.
    ...
    And I click the "/search?page=2&qf=TYPE%3A%22IMAGE%22&query=paris" link
    And I wait 1 second
    Then I should be on `/search?page=2&qf=TYPE%3A%22IMAGE%22&query=paris`
