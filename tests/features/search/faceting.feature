Feature: Search faceting
  ...
  Scenario: Filtering results by types
    ...
    Then I should be on `/search?page=1&qf=TYPE%3A%22IMAGE%22&query=`
  ...
  Scenario: Filtering results by reusability
    ...
    And I wait 2 seconds
    Then I should be on `/search?page=1&query=&reusability=open`
  ...
  Scenario: Filtering results by country
    ...
    And I wait 2 seconds
    Then I should be on `/search?page=1&qf=COUNTRY%3A%22Belgium%22&query=`
  ...
  Scenario: Filtering results by two countries
    ...
    And I wait 2 seconds
    Then I should be on `/search?page=1&qf=COUNTRY%3A%22Belgium%22&qf=COUNTRY%3A%22Germany%22&query=`
  ...
  Scenario: Filtering using a combination of facet fields
    ...
    And I check the "open" checkbox
    And I wait 2 seconds
    Then I should be on `/search?page=1&qf=COUNTRY%3A%22Belgium%22&qf=TYPE%3A%22IMAGE%22&query=&reusability=open`
  ...
  Scenario: Unselecting facets
    ...
    And I check the "open" checkbox
    And I wait 2 seconds
    Then I should be on `/search?page=1&query=`
