Feature: View styles (List and Grid)
  ...
  Scenario: The view paramater is preserved and present in the URL for the grid view
    Given I have chosen the `grid` search results view
    When I visit the `home page`
    And I click the `search button`
    And I wait 2 seconds
    Then I should be on `/search?query=&view=grid`

  Scenario: The view paramater is preserved and present in the URL for the list view
    Given I have chosen the `list` search results view
    When I visit the `home page`
    And I click the `search button`
    And I wait 2 seconds
    Then I should be on `/search?query=&view=list`
