Feature: Newspapers collection
  ...
  Scenario: Filtering results using the date filter in more facets dropdown
    ...
    Then I should be on `/en/entity/topic/18-newspapers?page=1&qf=proxy_dcterms_issued%3A%5B1982-05-18%20TO%202004-05-18%5D&query=&view=grid&api=fulltext`
  ...
  Scenario: Newspapers collection API toggle changes API
    ...
    And I click the `apply button`
    And I wait 2 seconds
    Then I should be on `/en/entity/topic/18-newspapers?page=1&query=&view=grid&api=metadata`
  ...
  Scenario: Newspapers collection API toggle is not removed when switching pages
    ...
    And I wait 2 seconds
    Then I should be on `/en/entity/topic/18-newspapers?page=2&query=&view=grid&api=metadata`
