...
<script>
  ...
  export default {
    ...
    computed: {
      manifest() {
        const manifestUrl = new URL(`/presentation${this.id}/manifest`, this.$apis.iiifPresentation.url);
        manifestUrl.searchParams.set('format', '3');
        manifestUrl.searchParams.set('recordApi', new URL(this.apis.record.url).origin);
