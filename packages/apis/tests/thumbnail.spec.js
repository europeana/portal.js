  ...
  describe('EuropeanaThumbnailApi', () => {
    describe('baseURL', () => {
      ...
      describe('Thumbnail API v2', () => {
        const config = { url: 'https://api.europeana.eu/thumbnail/v2' };

        it('is not supported', () => {
          expect(() => (new EuropeanaThumbnailApi(config))).toThrow();
        });
      });
    });
    ...
    describe('media()', () => {
      ...
      describe('Thumbnail API v3', () => {
        const config = { url: 'https://api.europeana.eu/thumbnail/v3' };

        describe('hash', () => {
          it('is used as-is when supplied', () => {
            const params = { hash: '123' };
            expect((new EuropeanaThumbnailApi(config)).media(uri, params).endsWith('/123')).toBe(true);
          });

          it('is generated from MD5 hash of URI when not supplied', () => {
            expect((new EuropeanaThumbnailApi(config)).media(uri).endsWith('/cffc370c6c63744ed934701a47b0349a')).toBe(true);
          });
        });
        ...
        describe('size', () => {
          it('is used as-is when supplied', () => {
            ...
            expect((new EuropeanaThumbnailApi(config)).media(uri, params).includes('/400/')).toBe(true);
          });

          it('defaults to 200 when not supplied', () => {
            expect((new EuropeanaThumbnailApi(config)).media(uri).includes('/200/')).toBe(true);
          });
        });
      });
    });
      ...
      describe('for Thumbnail API v3 edm:preview URL', () => {
        describe('with v3 API URL', () => {
          ...
          it('favours requested size', () => {
            ...
            const edmPreview = (new EuropeanaThumbnailApi(config)).edmPreview(url, { size: 400 });
